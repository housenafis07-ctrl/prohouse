...
begin
  ...
  for item in select * from public.monetization_order_items where order_id = o.id order by created_at, id for update loop
    ...
    if p.product_type in ('top', 'highlight', 'premium') and item.listing_id is not null then
      update public.listings
      set promoted_until = greatest(coalesce(promoted_until, start_time), start_time) + make_interval(days => greatest(p.duration_days, 1)),
      ...
      insert into public.listing_promotions(
        ...
        start_time,
        greatest(coalesce((select promoted_until from public.listings where id = item.listing_id), start_time), start_time),
        'active'
      );
    end if;
  end loop;
