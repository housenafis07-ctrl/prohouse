-- P0 security hardening: transaction actor authorization.
-- The API currently calls this RPC through the service-role client, so the
-- function must explicitly bind a supplied actor to the authenticated caller
...
begin
  ...
  if auth.role() <> 'service_role' and (auth.uid() is null or p_actor_id <> auth.uid()) then
    raise exception 'TRANSACTION_ACTOR_FORBIDDEN';
  end if;
...
revoke execute on function public.transition_property_transaction(uuid,text,uuid,text,jsonb) from public;
revoke execute on function public.transition_property_transaction(uuid,text,uuid,text,jsonb) from anon;
grant execute on function public.transition_property_transaction(uuid,text,uuid,text,jsonb) to authenticated;
grant execute on function public.transition_property_transaction(uuid,text,uuid,text,jsonb) to service_role;
