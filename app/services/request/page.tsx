...
const labels: Record<string, string> = { mortgage: 'Ipoteka', insurance: 'Sug‘urta', legal: 'Huquqiy tekshiruv', cadastral: 'Kadastr', property_service: 'Uy xizmatlari', property_valuation: 'Mulkni baholash' }
...
function ServiceRequestForm() {
  ...
  async function submit(e: FormEvent) {
    ...
    if (r.ok) { setMessage('So‘rovingiz qabul qilindi. Keyingi aloqa Royalhouse orqali davom etadi.'); setAmount(''); setDown(''); setTerm(''); setNotes('') }
    else setMessage(d.error || 'So‘rov yuborilmadi. Avval tizimga kirganingizni tekshiring.')
    setBusy(false)
  }

  return <main className="min-h-screen bg-slate-50 text-slate-900"><div className="mx-auto max-w-2xl px-4 py-10"><Link href="/services" className="text-sm font-bold text-emerald-700">← Xizmatlarga qaytish</Link><div className="mt-6 rounded-3xl bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-black uppercase tracking-widest text-emerald-600">Royalhouse Services</p><h1 className="mt-2 text-3xl font-black">Xizmat bo‘yicha so‘rov</h1><p className="mt-2 text-sm leading-6 text-slate-500">So‘rov yuboring. Sug‘urta va boshqa hamkorlik xizmatlari hozircha funnel bosqichida; hamkor kompaniyalar bilan shartnoma va API ma’lumotlari ulangach, onlayn rasmiylashtirish shu oqimga ulanadi.</p><form onSubmit={submit} className="mt-7 space-y-4"><label className="block text-sm font-bold">Xizmat turi<select value={type} onChange={e=>setType(e.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3">{Object.entries(labels).map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></label>{type==='mortgage'&&<div className="grid gap-4 sm:grid-cols-3"><label className="text-sm font-bold">Summa<input inputMode="decimal" value={amount} onChange={e=>setAmount(e.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="500 000 000" /></label><label className="text-sm font-bold">Boshlang‘ich to‘lov<input inputMode="decimal" value={down} onChange={e=>setDown(e.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="100 000 000" /></label><label className="text-sm font-bold">Muddat (oy)<input inputMode="numeric" value={term} onChange={e=>setTerm(e.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="240" /></label></div>}{(type==='insurance'||type==='property_valuation')&&<div className="rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-900">{type==='insurance'?'Sug‘urta kompaniyasi tanlash, tariflarni olish va polisni onlayn rasmiylashtirish uchun keyingi bosqichda hamkor API ulanadi.':'Baholovchi hamkor tanlash va ko‘chmas mulk qiymatini aniqlash bo‘yicha so‘rov qabul qilinadi.'}</div>}<label className="block text-sm font-bold">Telefon<input value={phone} onChange={e=>setPhone(e.target.value)} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="+998 90 123 45 67" /></label><label className="block text-sm font-bold">Izoh<textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={5} className="mt-2 w-full rounded-xl border px-4 py-3" placeholder="Qo‘shimcha ma’lumot..." /></label>{message&&<div className="rounded-xl bg-slate-100 p-4 text-sm font-semibold">{message}</div>}<button disabled={busy} className="w-full rounded-xl bg-emerald-600 px-5 py-3 font-black text-white disabled:opacity-50">{busy?'Yuborilmoqda...':'So‘rov yuborish'}</button></form></div></div></main>
}
