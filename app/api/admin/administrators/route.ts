...
export async function GET() {
  ...
  try {
    const supabase=serviceClient(); const [{data:roles,error:re},{data:permissions,error:pe},{data:users,error:ue}]=await Promise.all([
...
export async function POST(request:NextRequest){
 ...
 try{
  const body=await request.json(); const email=typeof body.email==='string'?body.email.trim().toLowerCase():''; const password=typeof body.password==='string'?body.password:''; const role=body.role==='admin'?'admin':'admin'; const permissionCodes=Array.isArray(body.permissionCodes)?body.permissionCodes.filter((x:any)=>typeof x==='string'):[]
  ...
  const {data:roleData,error:roleError}=await supabase.from('admin_roles').insert({user_id:userData.user.id,role,is_active:true}).select('id').single(); if(roleError)throw roleError
