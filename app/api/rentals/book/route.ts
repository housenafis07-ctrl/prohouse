...
export async function POST(request:NextRequest){
 try{
  ...
  if(maxGuests>0&&extraGuestFee<=0&&guests>maxGuests)return NextResponse.json({error:`Mehmonlar soni ${maxGuests} tadan oshmasin`},{status:400})
  ...
  const prices=settings.date_prices||{};const extraGuests=Math.max(0,guests-includedGuests);const total=requested.reduce((sum,d)=>{const override=num(prices[d]);const fallback=isWeekend(d)?num(settings.weekend_price):num(settings.weekday_price);const base=override||fallback||num(listing.price);return sum+base+(extraGuestFee*extraGuests)},0);const depositPercent=num(settings.deposit_percent)||15;const depositAmount=Math.round(total*depositPercent/100)
