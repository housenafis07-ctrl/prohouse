...
export async function POST(request:NextRequest){
 try{
  ...
  const prices=settings.date_prices||{};const total=requested.reduce((sum,d)=>{const override=Number(prices[d])||0;const fallback=isWeekend(d)?Number(settings.weekend_price||0):Number(settings.weekday_price||0);return sum+(override||fallback||Number(listing.price||0))},0);const depositPercent=Number(settings.deposit_percent||15);const depositAmount=Math.round(total*depositPercent/100)
