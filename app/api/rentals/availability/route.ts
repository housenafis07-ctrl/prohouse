...
export async function GET(request:NextRequest){
 try{
  ...
  const blocked=new Set<string>(settings.blocked_dates||[]);const prices=settings.date_prices||{};const days=eachDay(from,to).map(date=>({date,status:booked.has(date)?'booked':blocked.has(date)?'blocked':'free',price:prices[date]?Number(prices[date]):Number(listing.price||0)}))
  return NextResponse.json({listingId,depositPercent:Number(settings.deposit_percent||15),days})
