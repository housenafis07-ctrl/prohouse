...
function decodeCursor(value: string | null, sort: Cursor['sort']): Cursor | null {
  ...
  try {
    ...
    if (sort === 'newest' && parsed.published_at !== null && typeof parsed.published_at !== 'string') return null
    if (sort !== 'newest' && typeof parsed.price !== 'number') return null
    return parsed
  } catch {
    return null
  }
...
export async function GET(request: NextRequest) {
  ...
  const cursor = decodeCursor(params.get('cursor'), (params.get('sort') || 'newest') as Cursor['sort'])
  ...
  if (cursor) {
    if (sort === 'newest') {
      if (cursor.published_at === null) query = query.or(`published_at.is.null,and(published_at.is.null,id.lt.${cursor.id})`)
      else query = query.or(`published_at.lt.${cursor.published_at},and(published_at.eq.${cursor.published_at},id.lt.${cursor.id}),published_at.is.null`)
  ...
  const useCursor = Boolean(cursor) || !params.has('page')
  ...
  const nextCursor = hasNext && listings.length
    ? encodeCursor(sort === 'newest'
      ? { sort, id: listings[listings.length - 1].id, published_at: listings[listings.length - 1].published_at }
      : { sort, id: listings[listings.length - 1].id, price: listings[listings.length - 1].price })
    : null
