...
export default function OfferAdminPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!me.ok) throw new Error('Admin sifatida kirish talab qilinadi.')
      const admin = await me.json()
      if (admin.role !== 'super_admin') throw new Error('Oferta boshqaruvi faqat bosh admin uchun.')
      const res = await fetch('/api/admin/offer', { cache: 'no-store' })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Oferta yuklanmadi')
