...
export default function TrustedProfilePage() {
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4"><Link href="/account" className="text-sm font-extrabold text-emerald-700">← Shaxsiy kabinet</Link><Link href="/" className="text-sm font-bold text-slate-500">Royalhouse</Link></div></header>
    <div className="mx-auto max-w-4xl px-4 py-8 sm:py-12"><div className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200"><div className="bg-slate-900 px-6 py-8 text-white sm:px-8"><p className="text-sm font-semibold text-slate-300">Royalhouse xavfsizlik tizimi</p><h1 className="mt-2 text-3xl font-black">Ishonchli profil</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Ixtiyoriy tasdiqlash orqali foydalanuvchining haqiqiyligini kuchaytirish va e’lonlarda ishonchli sotuvchini ajratib ko‘rsatish.</p><div className="mt-5">{profile.trusted_profile && <TrustedBadge/>}</div></div>
      <div className="p-6 sm:p-8"><div className="flex flex-col gap-4 rounded-2xl border border-emerald-100 bg-emerald-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-emerald-600">Holat</p><h2 className="mt-1 text-xl font-black text-emerald-900">{meta.title}</h2><p className="mt-1 text-sm leading-6 text-emerald-800">{meta.text}</p></div>{profile.trusted_profile && <TrustedBadge small/>}</div>
        ...
        <p className="mt-6 text-xs leading-5 text-slate-400">MyID biometrik identifikatsiyasi faqat foydalanuvchining tasdiqlangan roziligi bilan amalga oshirilishi kerak. Real MyID ulanishi uchun Royalhouse MyID bilan shartnoma va beriladigan test/tijoriy credentials asosida konfiguratsiya qilinadi.</p>
      </div></div></div></main>
}
