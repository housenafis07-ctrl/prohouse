...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!supabase) return
    supabase.from('listings').select('*, listing_images(image_url,sort_order)').eq('status','active').order('is_featured',{ascending:false}).order('published_at',{ascending:false}).then(({ data }) => {
