...
export default function ListingGallery({ images, title }: { images: ListingImage[]; title: string }) {
  ...
  const nextImage = () => setActiveImage(i => setActiveImage(i => images.length ? (i + 1) % images.length : 0))
  ...
  const resolveSigned = async (image: ListingImage, index: number) => {
    ...
    try {
      ...
      const { data } = await db.storage.from('listing-images').createSignedUrl(path, 3600)
      if (data?.signedUrl) setResolved(state => ({ ...state, [index]: data.signedUrl }))
  ...
  return (
    <>
      ...
      {lightbox && current && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3 sm:p-8" role="dialog" aria-modal="true" aria-label={`${title} rasmlar galereyasi`} onClick={() => setLightbox(false)}>
        ...
        <img src={imageSrc(current, safeIndex)} alt={`${title} — ${safeIndex + 1}-rasm`} onError={() => void resolveSigned(current, safeIndex)} onClick={event => event.stopPropagation()} className="max-h-[90vh] max-w-[95vw] rounded-2xl object-contain shadow-2xl" />
