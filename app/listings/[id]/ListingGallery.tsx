...
export default function ListingGallery({ images, title }: { images: ListingImage[]; title: string }) {
  ...
  // Use the stored public URL directly on the detail page. This avoids broken
  // Supabase image-transform URLs when Image Transformations are not enabled.
  const imageSrc = (image: ListingImage, index: number) => failed[index] ? image.image_url : image.image_url

  return (
    <>
      <div className="bg-slate-100">
        <div className="relative h-[300px] cursor-zoom-in touch-pan-y select-none sm:h-[420px]" onClick={() => setLightbox(true)} onTouchStart={event => setTouchStart(event.touches[0]?.clientX ?? null)} onTouchEnd={event => {
          if (touchStart == null) return
          const delta = (event.changedTouches[0]?.clientX ?? touchStart) - touchStart
          if (Math.abs(delta) >= 50) delta < 0 ? nextImage() : previousImage()
          setTouchStart(null)
        }}>
          <img src={imageSrc(current, safeIndex)} alt={`${title} — ${safeIndex + 1}-rasm`} title={title} loading="eager" fetchPriority="high" decoding="async" draggable={false} onError={() => setFailed(state => ({ ...state, [safeIndex]: true }))} className="h-full w-full object-cover" />
          {images.length > 1 && <>
        ...
        {images.length > 1 && <div className="flex gap-2 overflow-x-auto border-t border-slate-200 bg-white p-3">
          {images.map((image, index) => <button key={`${image.image_url}-${index}`} type="button" onClick={() => setActiveImage(index)} aria-label={`${title} — ${index + 1}-rasm`} className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border-2 ${safeIndex === index ? 'border-emerald-500' : 'border-transparent'}`}><img src={image.image_url} alt={`${title} — ${index + 1}-rasm`} loading="lazy" decoding="async" draggable={false} onError={() => setFailed(state => ({ ...state, [index]: true }))} className="h-full w-full object-cover" /></button>)}
        </div>}
      </div>
      ...
      {lightbox && current && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-3 sm:p-8" role="dialog" aria-modal="true" aria-label={`${title} rasmlar galereyasi`} onClick={() => setLightbox(false)}>
        ...
        <img src={current.image_url} alt={`${title} — ${safeIndex + 1}-rasm`} onClick={event => event.stopPropagation()} className="max-h-[90vh] max-w-[95vw] rounded-2xl object-contain shadow-2xl" />
