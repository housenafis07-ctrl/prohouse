...
function Map({ latitude, longitude, price, currency, title }: { latitude: number | null; longitude: number | null; price: number; currency: string; title: string }) {
  ...
  useEffect(() => {
    ...
    const L = (window as any).L; const element = document.getElementById(mapId); if (!element) return
    const css = 'https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'
    if (!document.querySelector(`link[href="${css}"]`)) { const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = css; document.head.appendChild(link) }
    ...
    const formatted = new Intl.NumberFormat('ru-RU', { notation: 'compact', maximumFractionDigits: 1 }).format(price)
    const suffix = currency === 'USD' ? '$' : 'so‘m'
    const marker = L.marker(point, { icon: L.divIcon({ className: '', html: `<div style="background:#10b981;color:#fff;border:2px solid #fff;border-radius:999px;padding:7px 11px;box-shadow:0 3px 12px rgba(0,0,0,.2);font-size:12px;font-weight:800;white-space:nowrap">${formatted} ${suffix}</div>`, iconAnchor: [18, 18] }) }).addTo(map)
...
export default function ListingDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const { data, error } = await supabase.from('listings').select('*, listing_images(image_url,sort_order)').eq('id', params.id).eq('status', 'active').maybeSingle()
        if (error) throw error
        if (!data) throw new Error('E’lon topilmadi yoki hozir faol emas')
        if (mounted) { setListing(data as Listing); setActiveImage(0) }
      } catch (e: any) { if (mounted) setError(e?.message || 'E’lonni yuklashda xatolik yuz berdi.') }
  ...
  const safeImageIndex = Math.min(activeImage, Math.max(images.length - 1, 0))
  ...
  return <main className="min-h-screen bg-[#f6f7f8] text-slate-900">
    ...
    <div className="mx-auto max-w-[1200px] px-4 py-6 sm:px-6">
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="grid lg:grid-cols-[1.35fr_.65fr]"><div className="bg-slate-100">
        <div className="relative h-[420px] sm:h-[520px]">
          {images.length ? <img src={images[safeImageIndex].image_url} alt={title} className="h-full w-full object-cover"/> : <div className="flex h-full items-center justify-center text-slate-400">Rasm mavjud emas</div>}
          {images.length > 1 && <><button aria-label="Oldingi rasm" onClick={previousImage} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-white/90 px-4 py-3 text-xl font-black shadow-lg hover:bg-white">‹</button><button aria-label="Keyingi rasm" onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-white/90 px-4 py-3 text-xl font-black shadow-lg hover:bg-white">›</button><div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/65 px-3 py-1.5 text-xs font-bold text-white">{safeImageIndex + 1} / {images.length}</div></>}
        </div>
        {images.length > 1 && <div className="flex gap-2 overflow-x-auto border-t border-slate-200 bg-white p-3">{images.map((image, index) => <button key={`${image.image_url}-${index}`} onClick={() => setActiveImage(index)} className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border-2 ${safeImageIndex === index ? 'border-emerald-500' : 'border-transparent'}`}><img src={image.image_url} alt={`${title} — ${index + 1}`} className="h-full w-full object-cover"/></button>)}</div>}
      </div>
          <div className="p-6 sm:p-8"><div className="flex flex-wrap gap-2">{listing.is_featured && <span className="rounded-lg bg-amber-400 px-2.5 py-1 text-xs font-black">TOP</span>}{listing.is_verified && <span className="rounded-lg bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">✓ Tasdiqlangan</span>}{listing.is_trusted_seller && <TrustedBadge/>}</div><h1 className="mt-4 text-2xl font-black sm:text-3xl">{title}</h1><p className="mt-3 text-2xl font-black text-emerald-600">{money(listing.price, listing.currency)}</p><p className="mt-3 text-sm text-slate-500">⌖ {listing.city}{listing.district ? `, ${listing.district}` : ''}{listing.address ? `, ${listing.address}` : ''}</p><div className="mt-6 grid grid-cols-2 gap-3">{listing.area_m2 != null && <div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">Maydon</span><b className="mt-1 block">{listing.area_m2} m²</b></div>}{listing.rooms != null && <div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">Xonalar</span><b className="mt-1 block">{listing.rooms}</b></div>}{listing.floor != null && <div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">Qavat</span><b className="mt-1 block">{listing.floor}/{listing.floors_total}</b></div>}<div className="rounded-xl bg-slate-50 p-3"><span className="text-xs text-slate-400">Mulk turi</span><b className="mt-1 block">{typeLabel[listing.property_type] || listing.property_type}</b></div></div><div className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-bold text-emerald-700">Sotuvchi</p>{listing.is_trusted_seller && <TrustedBadge/>}</div><p className="mt-1 font-black">{listing.seller_name || 'Sotuvchi'}</p>{listing.is_verified && <p className="mt-1 text-xs text-emerald-700">✓ Tasdiqlangan profil</p>}{listing.is_trusted_seller && <p className="mt-1 text-xs font-semibold text-emerald-700">MyID orqali tasdiqlangan Ishonchli profil.</p>}</div></div></div></section>
