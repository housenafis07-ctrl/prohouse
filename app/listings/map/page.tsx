...
const REGION_CENTERS: Record<string, [number, number]> = {
  ...
  'Buxoro viloyati': [39.77, 64.43],
  'Jizzax viloyati': [40.12, 67.84],
  'Qashqadaryo viloyati': [38.86, 65.80],
  'Navoiy viloyati': [40.10, 65.38],
  'Namangan viloyati': [41.00, 71.67],
  'Samarqand viloyati': [39.65, 66.96],
  'Sirdaryo viloyati': [40.47, 68.78],
  'Surxondaryo viloyati': [37.94, 67.57],
  'Toshkent viloyati': [41.10, 69.30],
  'Toshkent shahri': [41.2995, 69.2401],
  'Farg‘ona viloyati': [40.39, 71.79],
  'Xorazm viloyati': [41.55, 60.63],
}

function MapView({ items, selected, onSelect, fallbackCenter }: { items: Listing[]; selected: string | null; onSelect: (id: string) => void; fallbackCenter: [number, number] }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!mapRef.current) {
        mapRef.current = L.map(ref.current).setView(fallbackCenter, 9)
      ...
      points.forEach(x => {
        ...
        const marker = L.marker([Number(x.latitude), Number(x.longitude)], {
          icon: L.divIcon({ className: '', html: `<div style="background:${active ? '#059669' : '#0f172a'};color:#fff;border:2px solid #fff;border-radius:999px;padding:6px 9px;box-shadow:0 3px 12px rgba(0,0,0,.22);font-size:11px;font-weight:800;white-space:nowrap">${new Intl.NumberFormat('ru-RU', { notation: 'compact', maximumFractionDigits: 1 }).format(Number(x.price))} ${x.currency === 'USD' ? '$' : 'so‘m'}</div>`, iconAnchor: [20, 20] }),
        }).addTo(layerRef.current)
        marker.on('click', () => onSelect(x.id))
      })
      if (points.length) {
        const bounds = L.latLngBounds(points.map(x => [Number(x.latitude), Number(x.longitude)]))
        mapRef.current.fitBounds(bounds, { padding: [40, 40], maxZoom: 13 })
      } else {
        mapRef.current.setView(fallbackCenter, 9)
      }
      setTimeout(() => mapRef.current?.invalidateSize(), 100)
    }
    void load()
    return () => { cancelled = true }
  }, [items, selected, onSelect, fallbackCenter])

  useEffect(() => {
    if (!mapRef.current || !selected) return
    const x = items.find(v => v.id === selected)
    if (x?.latitude != null && x.longitude != null) mapRef.current.flyTo([Number(x.latitude), Number(x.longitude)], 15, { duration: .5 })
  }, [selected, items])
...
export default function MapListingsPage() {
  ...
  const fallbackCenter = useMemo<[number, number]>(() => REGION_CENTERS[location.region] || REGION_CENTERS['Toshkent shahri'], [location.region])

  const queryString = useMemo(() => {
    const q = new URLSearchParams()
    if (mode !== 'all') q.set('tab', mode)
    if (type) q.set('property_type', type)
    if (location.region) q.set('region', location.region)
    if (location.district) q.set('district', location.district)
    if (location.neighborhood) q.set('neighborhood', location.neighborhood)
    return q.toString()
  }, [mode, type, location])
  ...
  const locationTitle = [location.region, location.district, location.neighborhood].filter(Boolean).join(' · ')
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto max-w-[1440px] px-4 py-5">
      ...
      <div className="grid overflow-hidden rounded-3xl border bg-white shadow-sm lg:grid-cols-[1.35fr_.65fr]">
